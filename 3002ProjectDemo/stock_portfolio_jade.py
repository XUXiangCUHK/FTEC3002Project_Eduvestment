...
def display_multiple_selection_bar(snp):
    ...
    checked = st.multiselect('Select stocks', list(symbols), default = ['GOOG', 'FB'])
    if len(checked) == 0:
...
def plotstocks(df):
    ...
    for stock in df.columns.values:
        if stock == 'optimized portfolio':
            alpha = 1
            lw = 3
        elif stock == 'your portfolio':
            alpha = 1
            lw = 3
        else:
            alpha = 0.3
...
def display_volatility_chat(data, dates, period):
    ...
    st.write("""## Portfolio daily returns and {} period rolling volatility""".format(period))
